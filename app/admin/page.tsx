...
export default function Home() {
  ...
  // Transformar ventas de DB a UI map (API route ya las transforma, solo aseguramos que la ruta exista por validación)
  const uiSales: Sale[] = useMemo(() => {
    return sales.map((sale: any) => {
      // Find the route if it stringified or missing, otherwise use what API returned
      const routeObj = typeof sale.route === 'object' && sale.route?.id
        ? sale.route
        : routes.find(r => r.id === (sale.route?.id || sale.route_id)) || { id: 1, from: "Desconocido", to: "Desconocido", price: 0 }
  ...
  useEffect(() => {
    ...
    for (const sale of uiSales) {
      if (!sale.date || !sale.route?.id || !sale.schedule) continue;

      const routeId = sale.route.id.toString()
      const schedule = sale.schedule
      const seatNumber = sale.seatNumber

      const key = getSeatMapKey(sale.date, routeId, schedule)
      if (!map[key]) map[key] = {}
      map[key][seatNumber] = sale
    }
    setSeatMap(map)
  }, [sales])
  ...
  const confirmDeleteSale = async () => {
    ...
    try {
      ...
      const key = getSeatMapKey(sale.date, sale.route?.id?.toString() || "1", sale.schedule)
      setSeatMap((prev) => {
      ...
      toast({
        ...
        description: `Venta ${sale.id} eliminada correctamente. El asiento ${sale.seatNumber?.toString().padStart(2, "0") || "00"} está ahora disponible.`,
      })
  ...
  const handleNewSale = async () => {
    ...
    try {
      ...
      // Transformar a UI Sale para el estado local
      const uiNewSale: Sale = {
        id: newSale.id,
        date: newSale.travel_date,
        time: new Date(newSale.created_at).toLocaleTimeString() || "00:00",
        passenger: {
      ...
      setSeatMap((prev) => ({
        ...
        [key]: {
          ...prev[key],
          [selectedSeat]: uiNewSale,
        },
      }))

      setLastSale(uiNewSale)
  ...
  // Calculations
  const todaySales = uiSales.filter((sale) => sale.date === new Date().toISOString().split("T")[0])
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-slate-900 dark:to-slate-800">
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          {/* Tabs para desktop */}
          <TabsList className="grid w-full grid-cols-6 hidden md:grid">
            ...
            <TabsTrigger value="sitio-web" disabled={!hasPermission(authState.user, "config")}>
              Sitio Web
            </TabsTrigger>
          </TabsList>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={async (e) => {
                                ...
                                if (value.length === 8) {
                                  ...
                                  const existingPassenger = uiSales.find(
                          ...
                          {passengerDni.length === 8 &&
                            ...
                            selectedSchedule &&
                            (() => {
                              const existingSale = uiSales.find(
                                (sale) =>
                              ...
                              return existingSale ? (
                                <div className="p-3 bg-amber-50 dark:bg-amber-950/50 border border-amber-200 dark:border-amber-800 rounded-lg">
                                  ...
                                  <p className="text-xs text-amber-600 dark:text-amber-300 mt-1">
                                    Asiento actual: {existingSale.seatNumber?.toString().padStart(2, "0") || "00"} | Boleta:{" "}
                                    {existingSale.id}
                                  </p>
                                </div>
                              ) : null
                        ...
                        <div className="grid grid-cols-1 gap-4">
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              disabled={
                                passengerDni.length === 8 && uiSales.some((s) => s.passenger.dni === passengerDni)
                              }
                            />
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              disabled={
                                passengerDni.length === 8 &&
                                uiSales.some((s) => s.passenger.dni === passengerDni) &&
                                passengerPhone !== ""
                              }
                            />
                            <p className="text-xs text-muted-foreground">
                        ...
                        {selectedSeat && selectedDriver && (
                          <div className="bg-green-50 dark:bg-green-950/50 p-4 rounded-lg border border-green-200 dark:border-green-800">
                            ...
                            <div className="text-sm space-y-1 text-green-700 dark:text-green-200">
                              <p>
                                <strong>Asiento:</strong> {selectedSeat?.toString().padStart(2, "0") || "00"}
                              </p>
                              <p>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {uiDrivers
                          .filter((d) => d.isActive)
                          .map((driver) => {
                            const driverSales = uiSales.filter((sale) => sale.driver.id === driver.id)
                      ...
                      {uiSales
                        ...
                        .map((sale) => (
                          <TableRow key={sale.id}>
                            ...
                            <TableCell>
                              <Badge variant="outline">{sale.seatNumber?.toString().padStart(2, "0") || "00"}</Badge>
                            </TableCell>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {routes
                      ...
                      .map((route) => {
                        ...
                        const available = getAvailableSeatsCount(seatMap, selectedDate, route.id?.toString() || "1", time)
          ...
          <TabsContent value="reportes" className="space-y-6">
            ...
            <Tabs defaultValue="general" className="space-y-4">
              ...
              <TabsContent value="general" className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Card>
                    ...
                    <CardContent>
                      {routes.map((route) => {
                        const routeSales = uiSales.filter(
              ...
              <TabsContent value="conductores">
                <PassengerByDriverReport sales={uiSales} routes={routes} drivers={uiDrivers} />
              </TabsContent>
            </Tabs>
          </TabsContent>

          {/* SITIO WEB TAB (CMS BUILDER) */}
          <TabsContent value="sitio-web">
            <SiteBuilder />
          </TabsContent>
        </Tabs>
      </div>    <DriverManagementModal
      ...
      <BulkWhatsAppModal
        ...
        sales={uiSales.slice(-20)} // Últimas 20 ventas
      />
